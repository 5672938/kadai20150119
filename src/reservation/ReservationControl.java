...
//施設概要
	public String getExplanation(String facility){	
		...
		try {
			...
			boolean exist = false;
			while(rs.next()){
				String open = rs.getString("open_time");
...
///予約の確認
	public String getConfirmOn( String reservation,String facility){
		...
		try {
			// 予約情報を取得するクエリ
			ResultSet rs = mysql.getConfirm(reservation,facility);
			boolean exist = false;
			while(rs.next()){
				String start = rs.getString("start_time");
...
//////予約のキャンセル
public String getCancelOn(MainFrame frame){
					...
					try {
						...
						ResultSet rs = mysql.selectReservation(rdate, facility);
					      ...
					      while(rs.next()){
						        ...
						        if ( (start.compareTo(st)<=0 && st.compareTo(start)<=0) &&		//レコードの開始時刻＜キャンセル予約の開始時刻　AND　キャンセル予約の開始時刻＜レコードの終了時刻
						        	 (end.compareTo(et)<=0 && et.compareTo(end)<=0)){		//キャンセル予約の開始時刻＜レコードの開始時刻　AND　レコードの開始時刻＜キャンセル予約の開始時刻
									 	// 重複有りの場合に ng をtrueに設定
						        	ng = true; break;
						        }
					      }
					      ...
					      if (!ng){	//重なっていない場合
					    	  ...
					    	  int rs_int = mysql.getCancel(rdate, st, et, res, facility);
					    	  res = "正常にキャンセルされました";
					      }
					}catch (Exception e) {
						e.printStackTrace();
