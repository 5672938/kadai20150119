...
public class MySQL {
	...
	public MySQL() {
		...
		this.dbname  = "50416017";         // データベース名;
		this.url = "jdbc:mysql://" + server + "/" + dbname + "?useUnicode=true&characterEncoding=UTF-8";
		this.user = "50416017";         // データベース作成ユーザ名;
		this.password  = "50416017";     // データベース作成ユーザパスワード;
...
public ResultSet getExplanation( String facility){
	...
	String sql = "SELECT * FROM facility WHERE facility_name = '"+ facility +"' ORDER BY open_time;";
	ResultSet rs = null;
	try {
		rs = stmt.executeQuery (sql);  //try catchで囲む
	} catch (SQLException e) {
...
//予約の確認
public ResultSet getConfirm( String reservation,String facility){
...
//予約のキャンセル
public int getCancel(String rdate, String st, String et, String reservation_userid, String facility){
	int rs_int = 0;
	String sql = "DELETE FROM reservation WHERE date ='" + rdate + "' AND facility_name = '"+ facility +"' AND start_time ='"+ st +"' AND end_time = '"+ et + "';";
	    	  try {
